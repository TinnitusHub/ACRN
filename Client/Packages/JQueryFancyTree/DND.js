...
function _initDragAndDrop(tree) {
	...
	if(dnd && dnd.dragStart )
	{
		tree.widget.element.draggable(
		{
			...
			containment: "body", // custom changed
			delay: 0,
			distance: 4,
...
//            logMsg("draggable-connectToFancytree.drag: targetNode(from event): %s, ftTargetNode: %s", targetNode, ui.helper.data("ftTargetNode"));
			...
			// Leaving a tree node
			if(prevTargetNode && prevTargetNode !== targetNode && prevTargetNode.tree.ext.dnd) { // custom changed
				prevTargetNode.tree.ext.dnd._onDragEvent("leave", prevTargetNode, sourceNode, event, ui, draggable);
			}
			if(targetNode){
				if(!(targetNode.tree.options.dnd || {}).dragDrop) { // custom changed
...
//            logMsg("    targetNode: %o", targetNode);
			...
			if (targetNode && targetNode.tree.ext.dnd) { // custom changed
				if(dropped){
