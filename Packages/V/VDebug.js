var VDebug = new function()
{
	...
	self.MarkTime = function(/*;optional*#/ prepend)
	{
		Log(prepend + ") " + (new Date().getTime() - timerStartTime));
	};
	self.MarkTime = function(/*;optional*#/ prepend)
	{
		Log(prepend + ") " + (new Date().getTime() - timerStartTime));
	...
	var timerStartTime = 0;
	self.StartBlock = function() { timerStartTime = new Date().getTime(); }
	self.MidBlock = function(name)
	{
		self.EndBlock(name);
		self.StartBlock();
	}
	self.EndBlock = function(name) { Log("Block_" + name + ") " + (new Date().getTime() - timerStartTime)); } // ElapsedMilliseconds
	self.Block = function(/*optional:*/ name)
	{
		if (name != null)
			self.MidBlock(name);
		else
